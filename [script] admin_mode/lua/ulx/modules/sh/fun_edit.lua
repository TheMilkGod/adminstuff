timer.Simple(8.1, function()
	...
	------------------------------ Jail ------------------------------
function ulx.jail(ply, target, seconds, reason, unjail)
	for i=1, #target do
		if unjail == false then
			local v = target[i]
			Jail(v, seconds)
			if reason then
				local str = "| #A teleported player #T to jail for #i seconds. Reason: #s"
				...
				local v = target[i]
				local str = "| #A pulled player #T out of jail"
...
jail:addParam{ type=ULib.cmds.NumArg, min=0, default=0, hint="seconds", ULib.cmds.round, ULib.cmds.optional }
...
jail:help( "Sends the player to jail" )
jail:setOpposite( "ulx unjail", {_, _, _, _, true}, "!unjail" )



function Jail(ply, seconds, after_relog)
	...
	ply:SetPos(Vector(-6853.710938, -1622.063232, 323.50805)) -- jail's position
	ply:StripWeapons()
...
hook.Add("PlayerSpawn","ulxSpawnInJailIfDead",function (ply)
	...
	if ply.jailed == true then
		timer.Simple(1,function ()
			ply:SetPos(Vector(2338.629639, 2529.904541, 128.031250))
		end)
	end
end)
